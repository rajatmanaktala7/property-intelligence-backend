...
def parse_message(row):
    ...
    inherited=merge_phones(phones(full),row.get("sender_phone"))
    out=[]
    for idx,seg in enumerate(atomic_segments(full),1):
        ...
        ph=merge_phones(phones(seg),inherited)
        locs=locations(seg)
        ...
        out.append({"rtype":rtype,"raw":seg,"phone":ph[0] if ph else "","phones":ph,"tx":detect_transaction(seg,rtype),"ptype":ptype,"locs":locs,"area":ar,"budget":bu,"confidence":min(conf,100),"status":status,"reason":reason,"idx":idx})
    return out
            ...
            try:
                for rec in parse_message(row):
                    ...
                    c.execute(text("""INSERT INTO wai_clean_records(
                        ...
                        rejection_reason,source_fingerprint,source_created_at)
                        VALUES(:id,:mid,:grp,:rtype,:tx,:raw,:phone,CAST(:phones AS jsonb),:btxt,:bmin,:bmax,:bperiod,
                        :atxt,:amin,:amax,:aunit,:asmin,:asmax,:loc,CAST(:locs AS jsonb),:ptype,:person,:conf,:status,:reason,:fp,:sent)
                    ...
                    {"id":uuid.uuid4(),"mid":row["source_message_id"],"grp":row["source_group"] or "","rtype":rec["rtype"],"tx":rec["tx"],"raw":rec["raw"],
                     ...
                     "locs":json.dumps(locs),"ptype":rec["ptype"],"person":row["sender_display_name"] or "","conf":rec["confidence"],
                     "status":rec["status"],"reason":rec["reason"],"fp":fp,"sent":row["sent_at"]})
...
def dashboard():
    ...
    <div class=card><b>Lead rule:</b> the active Inventory, Requirements and AI Match Database show only records that have contact details.
...
def clean_matches():
    ...
    with engine.begin() as c:
        ...
        reqs=c.execute(text("""SELECT * FROM wai_clean_records WHERE record_type='REQUIREMENT'
            AND COALESCE(contact_no,'')<>'' AND status<>'inactive' ORDER BY source_created_at DESC NULLS LAST LIMIT 1000""")).mappings().all()
        supplies=c.execute(text("""SELECT * FROM wai_clean_records WHERE record_type='INVENTORY'
            AND COALESCE(contact_no,'')<>'' AND status<>'inactive' ORDER BY source_created_at DESC NULLS LAST LIMIT 3000""")).mappings().all()
    rows=[]
    for q in reqs:
        for p in supplies:
