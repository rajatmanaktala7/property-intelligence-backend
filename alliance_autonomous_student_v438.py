...
MODE = "RESTORE_V435_STABLE_BASE_APPLY_DEMAND_OBJECT_SEMANTICS_V5_STILL_PROTECTED"
...
def _asset_or_accommodation(s):
    return bool(re.search(
        r"\b(?:bhk|flat|apartment|farm\s*house|house|hotel|guest\s*house|hostel|dormitory|"
        r"property|building|plot|shop|office|villa|floor|showroom|warehouse|land|space|"
        r"staff\s+accommodation|accommodation)s?\b", s or ""))

def _occupant_object(s):
    return bool(re.search(
        r"\b(?:good\s+tenant|tenant|tenants|buyer|buyers|occupant|occupants|client|clients|"
        r"staff\s+member|staff\s+members)\b", s or ""))

def leading_demand_object(raw):
    cs=_clauses(raw)
    if not cs:
        return False
    ...
    third=_strip_symbols(cs[2]) if len(cs)>2 else ""

    # Strong demand verbs/nouns. These can own a CRE demand heading.
    strong=bool(re.match(
        r"^(?:urgent(?:ly)?\s+)?(?:required?|requirement|require|requires|need(?:ed)?|"
        r"wanted|seeking|tenant\s+requirement|buyer\s+required)\b", first))

    # "Looking for" is ambiguous: looking for a tenant is supply-side,
    # while looking for a flat/office/property is demand-side.
    looking=bool(re.match(r"^looking\s+for\b", first))
    looking_asset=looking and _asset_or_accommodation(first) and not _occupant_object(first)

    # Client/tenant/buyer phrases are demand only if they directly own a CRE
    # asset or a transaction in the same clause.
    actor=bool(re.match(r"^(?:client|tenant|buyer)\s+(?:requires?|needs?|seeks?|wants?)\b", first))
    actor_owned=actor and (
        _asset_or_accommodation(first) or
        bool(re.search(r"\b(?:purchase|buy|acquire|rent|lease)\b", first))
    )

    # A heading such as "REQUIRED - STAFF ACCOMMODATION" is complete demand
    # evidence even when the concrete asset choices appear several lines later.
    heading_asset=_asset_or_accommodation(first)
    near_asset=_asset_or_accommodation(second) or _asset_or_accommodation(third)

    if strong:
        return heading_asset or near_asset
...
def _direct_rental(raw):
    return v426.rental_contract(raw) or bool(re.search(r"\brental\s+requirement\b", _norm(raw)))
...
def predict_message(raw):
    ...
    # 4.3.5 remains the stable base because it retained the mature
    # offer-vs-demand and low-budget transaction behavior.
    p=v435.predict_message(raw)
    out=dict(p)
    rules=[p.get("rule","V435_BASE")]

    demand=leading_demand_object(raw)
    ...
    if demand and out["class"]!="NOISE":
        ...
        rules.append("V438_DEMAND_OBJECT_OWNS_CLASS")

        if sale:
            out["transaction"]="SALE"
            rules.append("V438_DEMAND_OBJECT_EXPLICIT_SALE")
        elif rent:
            out["transaction"]="RENT"
            rules.append("V438_DEMAND_OBJECT_EXPLICIT_RENT")
        elif rf["low_budget"] and rf["residential_spec"] and not rf["plural_capital"]:
            out["transaction"]="RENT"
            rules.append("V438_DEMAND_OBJECT_MONTHLY_BUDGET_RENT")
        elif rf["plural_capital"]:
            out["transaction"]="SALE"
            rules.append("V438_DEMAND_OBJECT_CAPITAL_BUDGET_SALE")
        else:
            out["transaction"]="UNKNOWN"
            rules.append("V438_DEMAND_OBJECT_TRANSACTION_ABSTAIN")

    out["rule"]="|".join(x for x in rules if x)
    evidence=dict(out.get("evidence") or {})
    evidence["v438"]={
        "leading_demand_object":demand,
        "direct_rental":rent,
        "direct_sale":sale,
        "stable_base":"4.3.5",
        "policy":"Demand must own a CRE asset/accommodation object. Looking for tenant is supply-side and cannot override availability."
    }
...
CLOSURE_REGRESSION=[
    ("required_staff_accommodation",
     "REQUIRED - STAFF ACCOMMODATION\nLooking for staff accommodation for 60-70 members.\nHotel / Guest House / Hostel / Dormitory / Large residential property\nIf you have a suitable building available, please send details.",
     "REQUIREMENT","UNKNOWN","OWNED"),
    ("requirement_unrelated_lease",
     "Requirement house 300 sq yd in GK2. Separate office lease portfolio also available.",
     "REQUIREMENT","UNKNOWN","OWNED"),
    ("requirement_unrelated_rent_business",
...
def training_status(engine):
    ...
    return {
        ...
        "training_gate":"V438_TRAINING_PASS_READY_FOR_FRESH_V5_FREEZER" if passed else "V438_TRAINING_HOLD_V5_NOT_FROZEN",
        "v5":{"status":"NOT_FROZEN"},
        "scientific_policy":"4.3.5 stable base plus demand-object semantics. Fresh V5 remains untouched until cumulative PASS.",
        "safety":{"v5_freeze":0,"production_writes":0,"whatsapp_writes":0,"gold_mutations":0},
    }

def _dashboard(engine):
    s=training_status(engine)
    return f"""<!doctype html><html><head><meta charset='utf-8'><title>Alliance Student 4.3.8</title>
...
<h1>Alliance Student 4.3.8 — Demand Object Closure</h1>
<div class='gate'>{html.escape(s["training_gate"])}</div>
<p>Demand must own the CRE asset/accommodation object. Looking for a tenant is supply-side. Fresh V5 remains untouched.</p>
...
def register(core):
    ...
    if not foundation._route_exists(app,"/api/property-brain/autonomous-v438/status"):
        @app.get("/api/property-brain/autonomous-v438/status")
        def status_v438(): return training_status(engine)
    if not foundation._route_exists(app,"/property-brain/autonomous-v438"):
        @app.get("/property-brain/autonomous-v438",response_class=HTMLResponse)
        def page_v438(): return HTMLResponse(_dashboard(engine))
    return {"status":"REGISTERED","version":VERSION,"route":"/property-brain/autonomous-v438",
