...
from fastapi import Request, Body, UploadFile, File, Form, HTTPException
from fastapi.responses import HTMLResponse, Response
from sqlalchemy import text

MODULE_VERSION="3.6.2-FAIL-SAFE-LIVE-TEAM-OPERATIONS"
HTML_FILE=Path(__file__).with_name("alliance_v36_dashboard.html")

DATASETS={
 "manual":{"table":"ai_team_live_record","id":"record_id",
   "fields":{"source_channel","record_type","business_name","contact_name","phone","email","location","details","status","assigned_to"}},
 "whatsapp":{"table":"wai_clean_records","id":"id",
 ...
 "newspaper":{"table":"pi_newspaper_properties","id":"id",
 ...
 "hospitality":{"table":"ai_hospitality_entity","id":"hospitality_id",
 ...
 "retail_signal":{"table":"ai_retail_expansion_signal","id":"signal_id",
 ...
 "retail_contact":{"table":"ai_retail_contact","id":"retail_contact_id",
...
def _serialize(rows):
    ...
    for r in rows:
        ...
        for k,v in list(d.items()):
            ...
            elif isinstance(v,(bytes,bytearray)): d[k]=None
        out.append(d)
    return out
...
def ensure_live_schema(engine):
    with engine.begin() as c:
        ...
        c.execute(text("""CREATE TABLE IF NOT EXISTS ai_team_live_record(
          record_id BIGSERIAL PRIMARY KEY, source_channel TEXT NOT NULL,
          record_type TEXT,business_name TEXT,contact_name TEXT,phone TEXT,email TEXT,
          location TEXT,details TEXT,status TEXT NOT NULL DEFAULT 'UNVERIFIED',
          assigned_to TEXT,created_by TEXT,created_at TIMESTAMPTZ DEFAULT NOW(),
          updated_at TIMESTAMPTZ DEFAULT NOW())"""))
        c.execute(text("""CREATE TABLE IF NOT EXISTS ai_team_live_media(
          media_id BIGSERIAL PRIMARY KEY,
          record_id BIGINT REFERENCES ai_team_live_record(record_id) ON DELETE CASCADE,
          filename TEXT,mime_type TEXT,content BYTEA NOT NULL,
...
def _dataset_rows(engine,dataset,limit=100,source_channel=""):
    ...
    with engine.connect() as c:
        if dataset=="manual":
            ...
            sql="SELECT * FROM ai_team_live_record"
            params={"lim":limit}
            if source_channel:
                sql+=" WHERE source_channel=:src"; params["src"]=source_channel.upper()
            sql+=" ORDER BY record_id DESC LIMIT :lim"
            return _serialize(c.execute(text(sql),params).mappings().all())
        cfg=DATASETS[dataset]
        if not _table_exists(c,cfg["table"]): return []
        return _serialize(c.execute(
            text(f'SELECT * FROM "{cfg["table"]}" ORDER BY "{cfg["id"]}" DESC LIMIT :lim'),
            {"lim":limit}).mappings().all())

def mount_features(core):
    global _FEATURES_MOUNTED,_MOUNT_ERRORS
    if _FEATURES_MOUNTED:
        return {"mounted":False,"already_mounted":True,"errors":_MOUNT_ERRORS}
    ...
    def mount_manual():
        ...
        def add_manual(req:Request,payload:dict=Body(...)):
            ...
            if src not in {"MANUAL","WHATSAPP_GROUP","NEWSPAPER","MAGAZINE","PHONE_IMPORT","OTHER"}: src="MANUAL"
            with engine.begin() as c:
                rid=c.execute(text("""INSERT INTO ai_team_live_record(
    ...
    def mount_upload():
        ...
        async def upload(req:Request,file:UploadFile=File(...),
            ...
            with engine.begin() as c:
                rid=c.execute(text("""INSERT INTO ai_team_live_record(
                  source_channel,record_type,business_name,contact_name,phone,email,
                  location,details,status,assigned_to,created_at,updated_at)
                  VALUES(:src,:rt,:bn,:cn,:ph,:em,:loc,:det,'UNVERIFIED',:asgn,NOW(),NOW())
                  RETURNING record_id"""),
                  {"src":src,"rt":record_type,"bn":business_name,"cn":contact_name,
                   "ph":phone,"em":email,"loc":location,"det":details,"asgn":assigned_to}).scalar_one()
                mid=c.execute(text("""INSERT INTO ai_team_live_media(
                  record_id,filename,mime_type,content,created_at)
    ...
    def mount_update():
        ...
        def update(dataset:str,row_id:str,req:Request,payload:dict=Body(...)):
            ...
            if dataset not in DATASETS: raise HTTPException(404,"Unknown dataset")
            cfg=DATASETS[dataset]
            vals={k:v for k,v in payload.items() if k in cfg["fields"]}
            ...
            if dataset in {"manual","newspaper","hospitality","retail_contact"}: sets.append("updated_at=NOW()")
            with engine.begin() as c:
    ...
    def mount_delete():
        ...
        def delete(dataset:str,row_id:str,req:Request):
            ...
            return {"version":MODULE_VERSION,"status":"DELETED","rows_deleted":result.rowcount}
    guard("delete",mount_delete)
            ...
            try:
                with engine.connect() as c:
                    if _table_exists(c,"ai_v30_orchestrator_run"):
                        runs=_serialize(c.execute(text("""SELECT run_id,requirement_code,run_status,current_step,
                          next_step,requires_human_review,started_at,completed_at
                          FROM ai_v30_orchestrator_run ORDER BY run_id DESC LIMIT 20""")).mappings().all())
            except Exception as e:
                return {"version":MODULE_VERSION,"runs":[],"error":str(e)}
            return {"version":MODULE_VERSION,"runs":runs}
    guard("pipeline",mount_pipeline)
    ...
    guard("dashboard",mount_dashboard)

    _MOUNT_ERRORS=errors
    _FEATURES_MOUNTED=True
    return {"mounted":True,"errors":errors,"features_ok":[x for x in
      ["setup","data","manual","upload","media","update","delete","pipeline","dashboard"] if x not in errors]}
