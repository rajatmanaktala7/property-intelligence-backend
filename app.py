...
def database_page(req:Request,table_name:str=Query("properties")):
    role=need_login(req)
    ...
    if table_name=="properties":
        for row in rows:
            ...
            row["action"]=f'<a class="btn" href="/property/edit/{quote_plus(pid)}">Edit</a>' if pid else ""
    ...
    return HTMLResponse(ui_shell(role,"Database",tabs+f"<p>{len(rows)} records</p>"+table_html))
...
def admin_page(req:Request,table_name:str=Query("sources")):
    ...
    allowed=["sources","ai_jobs","verification","batches","media"]
    if table_name not in allowed:
        table_name="sources"
...
def workspace(req:Request,message:str=Query("")):
    ...
    <div class="card">
      ...
      <small>AI processing continues in the background for photos and PDFs.</small>
    </div>"""
    ...
    body=notice+'<div class="grid">'+upload_card+property_card+requirement_card+verify_card+match_card+'</div>'
