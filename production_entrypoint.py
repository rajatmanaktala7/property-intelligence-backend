...
VERSION = "5.0-CORE-V2-ROUTE-FREEZE"
...
def _load_core():
    ...
    try:
        ...
        # ALLIANCE_CORE_V2_ROUTE_FREEZE
        # The legacy bootstrap previously registered dozens of overlapping
        # generations (v820/v910/v1191/v1210/v124xx, rectifiers, route rescues,
        # restorers and UI patches) into the same FastAPI app. A later registrar
        # could silently replace behavior fixed by an earlier one.
        #
        # Core V2 freezes the production control plane to the three isolated
        # authorities created above. Existing source tables/data remain intact.
        # No database migration or destructive cleanup is performed here.
        stabilization = {
            "registered": True,
            "architecture": "CORE_V2_ROUTE_FREEZE",
            "legacy_registration_chain": "DISABLED",
            "database_authority": "alliance_final_5x5_databases_v910",
            "requirement_authority": "alliance_requirement_restore_v1235",
            "matcher_authority": "alliance_master_requirement_authority_v1",
        }

        # Keep the small navigation shell only. It does not own database,
        # requirement or matcher routes.
        try:
        ...
        # Explicit route-contract audit. Fail boot rather than serving a mixed
        # generation when an isolated authority is unavailable.
        if DATABASE_APP is None or REQUIREMENT_APP is None or MATCHER_APP is None:
            raise RuntimeError(
                "CORE_V2_AUTHORITY_MISSING:"
                f" database={DATABASE_APP is not None}"
                f" requirement={REQUIREMENT_APP is not None}"
                f" matcher={MATCHER_APP is not None}"
            )
