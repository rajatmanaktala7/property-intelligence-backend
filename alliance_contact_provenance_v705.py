...
VERSION="7.0.5-ALLIANCE-CONTACT-PROVENANCE-CLOSURE"
MODE="LOCK_V704_SCHEMA_ROLES_EXACT_CONTACT_COLUMNS_SOURCE_ID_LINKAGE_NO_AI_NO_SOURCE_MUTATION"
...
DDL=[
"""CREATE TABLE IF NOT EXISTS pi_source_aware_clean_v705(
...
"""CREATE INDEX IF NOT EXISTS idx_source_aware_v705_source ON pi_source_aware_clean_v705(source_type,source_table)""",
"""CREATE INDEX IF NOT EXISTS idx_source_aware_v705_fp ON pi_source_aware_clean_v705(identity_fingerprint)""",
"""CREATE TABLE IF NOT EXISTS pi_source_aware_canonical_v705(
...
"""CREATE TABLE IF NOT EXISTS pi_source_aware_links_v705(
...
"""CREATE TABLE IF NOT EXISTS pi_source_aware_exception_queue_v705(
 id BIGSERIAL PRIMARY KEY, source_type TEXT NOT NULL, source_table TEXT NOT NULL, source_pk TEXT NOT NULL,
...
"""CREATE TABLE IF NOT EXISTS pi_source_aware_runs_v705(
...
PHONE_ALIASES=["phone","contact","contact_no","contact_number","mobile","mobile_no","mobile_number",
"owner_contact","broker_contact","general_contact","contact_phone","phone_number","contact_numbers","phones",
"phone_numbers","valid_mobiles","valid_landlines","partial_contacts","normalized_contact","normalized_contacts",
"contact_number","raw_owner_contact","raw_broker_contact","raw_contact_number","owner_contact_normalized",
"broker_contact_normalized","general_contact_normalized"]
...
def _build_contact_index(engine):
    """
    Contact provenance closure based on live schema evidence:
      pi_magazine_master.source_id -> pi_magazine_contact_links.source_id
      pi_magazine_contact_links.normalized_contact -> canonical phone
    Also keeps conservative support for other known contact tables.
    """
    index=defaultdict(lambda:{"phones":[],"provenance":[]})
    audit=[]

    # Exact magazine relationship found by v7.0.4 forensic schema audit.
    if _table_exists(engine,"pi_magazine_contact_links"):
        cols=[x["name"] for x in _columns(engine,"pi_magazine_contact_links")]
        rows=_read_rows(engine,"pi_magazine_contact_links",False)
        linked_rows=0; linked_phones=0
        for _,row in rows:
            sid=row.get("source_id")
            vals=_phones_from_any(row.get("normalized_contact"))
            # Fallback to raw_evidence only if normalized_contact did not yield a valid number.
            if not vals:
                vals=_phones_from_any(row.get("raw_evidence"))
            if sid not in (None,"") and vals:
                k=("pi_magazine_master",str(sid))
                index[k]["phones"].extend(vals)
                index[k]["provenance"].append({
                    "kind":"MAGAZINE_SOURCE_ID_LINK",
                    "table":"pi_magazine_contact_links",
                    "source_id":str(sid),
                    "column":"normalized_contact",
                    "phones":vals
                })
                linked_rows+=1; linked_phones+=len(vals)
        audit.append({
            "table":"pi_magazine_contact_links",
            "relationship":"pi_magazine_master.source_id = pi_magazine_contact_links.source_id",
            "contact_column":"normalized_contact",
            "rows":len(rows),
            "rows_with_recovered_contact":linked_rows,
            "recovered_phone_values":linked_phones,
            "columns":cols
        })

    # Conservative generic fallback for any other contact tables.
    for t in [x for x in CONTACT_TABLE_CANDIDATES if x!="pi_magazine_contact_links" and _table_exists(engine,x)]:
        cols=[x["name"] for x in _columns(engine,t)]
        fks=_foreign_keys(engine,t)
        rows=_read_rows(engine,t,False)
        id_cols=[c for c in cols if c.lower() in {
            "source_id","property_id","magazine_id","magazine_property_id","listing_id",
            "record_id","master_id","ref_id","reference_id"
        }]
        for fk in fks:
            if fk["foreign_table"]=="pi_magazine_master" and fk["column"] not in id_cols:
                id_cols.append(fk["column"])
        phone_cols=[c for c in cols if any(k in c.lower() for k in ["phone","mobile","contact"])]
        recovered=0
        for _,row in rows:
            phones=[]
            for pc in phone_cols:
                phones.extend(_phones_from_any(row.get(pc)))
            ...
            for ic in id_cols:
                ...
                if key not in (None,""):
                    ...
                    index[k]["provenance"].append({
                        "kind":"LINKED_CONTACT_TABLE","table":t,"id_column":ic,"phones":phones
                    })
                    recovered+=1
        audit.append({"table":t,"columns":cols,"foreign_keys":fks,"rows":len(rows),"linked_recoveries":recovered})

    for k,v in index.items():
        v["phones"]=sorted(dict.fromkeys(v["phones"]))
        # Keep provenance compact/deterministic.
        seen=set(); out=[]
...
def run_once(core):
    ...
    try:
        ...
        with engine.begin() as c:
            c.execute(text("TRUNCATE pi_source_aware_links_v705, pi_source_aware_canonical_v705, pi_source_aware_exception_queue_v705, pi_source_aware_clean_v705 RESTART IDENTITY"))
        source_counts=Counter();status_counts=Counter();issue_counts=Counter();contact_provenance_counts=Counter();total_entity_rows=0;processed=0
        ...
        for t,base,filter_source in plan:
            ...
            for spk,raw in rows:
                ...
                for _p in prov: contact_provenance_counts.update([str(_p.get("kind") or "UNKNOWN")])
                with engine.begin() as c:
                    c.execute(text("""INSERT INTO pi_source_aware_clean_v705(
                    ...
                    if qstatus=="EXCEPTION":
                        c.execute(text("""INSERT INTO pi_source_aware_exception_queue_v705(source_type,source_table,source_pk,issues,clean_record)
        ...
        with engine.begin() as c:
            c.execute(text("""UPDATE pi_source_aware_clean_v705 s SET duplicate_group='DUP-'||substr(s.identity_fingerprint,1,12)
              WHERE s.identity_fingerprint IS NOT NULL AND
              (SELECT COUNT(*) FROM pi_source_aware_clean_v705 x WHERE x.identity_fingerprint=s.identity_fingerprint)>1"""))
            rows=c.execute(text("SELECT * FROM pi_source_aware_clean_v705 ORDER BY id")).mappings().all()
        cmap={};links=0
        for r in rows:
            ...
            if not cid:
                ...
                with engine.begin() as c:
                    c.execute(text("""INSERT INTO pi_source_aware_canonical_v705(
                ...
                with engine.begin() as c:c.execute(text("UPDATE pi_source_aware_canonical_v705 SET source_count=source_count+1,updated_at=NOW() WHERE canonical_id=:cid"),{"cid":cid})
            with engine.begin() as c:
                c.execute(text("""INSERT INTO pi_source_aware_links_v705(canonical_id,source_type,source_table,source_pk,source_row_hash)
            ...
            links+=1
        with engine.connect() as c:
            canon=c.execute(text("SELECT COUNT(*) FROM pi_source_aware_canonical_v705")).scalar_one()
            dgroups=c.execute(text("SELECT COUNT(DISTINCT duplicate_group) FROM pi_source_aware_clean_v705 WHERE duplicate_group IS NOT NULL")).scalar_one()
            drows=c.execute(text("SELECT COUNT(*) FROM pi_source_aware_clean_v705 WHERE duplicate_group IS NOT NULL")).scalar_one()
            with_contact=c.execute(text("SELECT COUNT(*) FROM pi_source_aware_clean_v705 WHERE jsonb_array_length(phones)>0")).scalar_one()
            props=c.execute(text("SELECT COUNT(*) FROM pi_source_aware_clean_v705 WHERE entity_type='PROPERTY_AVAILABILITY'")).scalar_one()
            reqs=c.execute(text("SELECT COUNT(*) FROM pi_source_aware_clean_v705 WHERE entity_type='REQUIREMENT'")).scalar_one()
            unknown=c.execute(text("SELECT COUNT(*) FROM pi_source_aware_clean_v705 WHERE entity_type='UNKNOWN'")).scalar_one()
            tx_unknown=c.execute(text("SELECT COUNT(*) FROM pi_source_aware_clean_v705 WHERE canonical_transaction='UNKNOWN'")).scalar_one()
        result={"version":VERSION,"mode":MODE,"status":"COMPLETE","schema_forensics":existing,"contact_recovery_audit":contact_audit,
                ...
                "counts":{"entity_source_rows":total_entity_rows,"clean_rows":processed,"canonical_rows":canon,"source_links":links,
                          ...
                          "by_contact_provenance":dict(contact_provenance_counts)},
                ...
                "repair_principles":{"source_tables_mutated":False,"city_missing_is_not_failure":True,"price_missing_is_not_failure":True,
                                     ...
                                     "contacts_recovered_from_direct_and_linked_tables":True,"magazine_source_id_contact_link_explicit":True,"strong_dedupe_only":True,"weak_duplicates_preserved_separately":True,
                                     "ai_calls_used":0,"promotion_to_master_performed":False},
                "next_gate":"AUDIT_PROMOTION_READY_THEN_BUILD_SAFE_MASTER_PROMOTION_V710"}
        with engine.begin() as c:
            c.execute(text("INSERT INTO pi_source_aware_runs_v705(version,mode,status,result) VALUES(:v,:m,'COMPLETE',CAST(:r AS JSONB))"),
...
<meta name='viewport' content='width=device-width,initial-scale=1'><title>Contact Provenance Closure 7.0.5</title>
...
<body><header><b>Alliance Newspaper + Magazine Contact Provenance 7.0.5</b><br>
<small>Exact contact columns · magazine source_id linkage · newspaper phone_numbers · zero AI calls</small></header>
...
def register(core):
    app=_app(core)
    if not _route_exists(app,"/api/property-brain/contact-provenance-v705/status"):
        @app.get("/api/property-brain/contact-provenance-v705/status")
        def _s():return status(core)
    if not _route_exists(app,"/property-brain/contact-provenance-v705"):
        @app.get("/property-brain/contact-provenance-v705",response_class=HTMLResponse)
...
def start(core):
    ...
    if not _STARTED:
        _STARTED=True
        threading.Thread(target=_runner,args=(core,),daemon=True,name="contact-provenance-v705").start()
    return STATE
