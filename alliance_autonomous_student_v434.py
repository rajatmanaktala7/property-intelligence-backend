...
MODE = "CUMULATIVE_DEMAND_FIRST_CLASS_OWNERSHIP_AND_STRICT_RENT_REQUIREMENT_REPAIR_THEN_FRESH_V5"
RULESET_VERSION = "CRE_STUDENT_2026_09_03_V434"
V4_EXAM_VERSION = v410.EXAM_VERSION
V5_EXAM_VERSION = "BLIND_AUDIT_V5_434_2026_09_03"
V5_TARGET = 20
OVERALL_PASS = 95.0
FIELD_PASS = 90.0

DDL = [
"""CREATE TABLE IF NOT EXISTS alliance_student_v434_runs(
...
"""CREATE TABLE IF NOT EXISTS alliance_v5_434_manifest(
...
"""CREATE TABLE IF NOT EXISTS alliance_v5_434_cases(
...
"""CREATE TABLE IF NOT EXISTS alliance_v5_434_truth(
...
"""CREATE TABLE IF NOT EXISTS alliance_v5_434_results(
...
def _first_clause(raw):
    src=(raw or "").strip()
    if not src:
        return ""
    parts=[re.sub(r"\s+"," ",c).strip() for c in re.split(r"[\n\r.;!?|]+",src) if c.strip()]
    return parts[0].lower() if parts else re.sub(r"\s+"," ",src).lower()

def _demand_first(raw):
    c=_first_clause(raw)
    return bool(re.match(
        r"^(?:urgent(?:ly)?\s+)?(?:require|requires|required|requirement|wanted|need(?:ed)?|"
        r"looking for|seeking|client requires?|client needs?|buyer required|tenant requirement)\b", c))

def _direct_rent_requirement(raw):
    src=(raw or "").lower()
    ...
    direct_pat=re.compile(
        r"(?:\b(?:require|requires|required|requirement|wanted|need(?:ed)?|looking for)\b.{0,45}\b(?:on rent|for rent|to rent|rental requirement|tenant requirement)\b|"
        r"\b(?:on rent|for rent|to rent)\b.{0,45}\b(?:require|requires|required|requirement|wanted|need(?:ed)?)\b|"
        r"\btenant requirement\b)")
    return any(req_pat.search(c) and direct_pat.search(c) for c in clauses)

def _explicit_offer_anchors(raw):
    n=_norm(raw)
    # Offer anchors that can coexist with words such as "required" or "client wants"
    # inside broker descriptions. These indicate the message owns an available asset.
    return bool(re.search(
        r"\b(?:available|avl|for sale|for rent|available for lease|available on lease|"
        r"to[- ]?let|asking rent|asking price|demand\s*(?:[:@-]|is)?\s*(?:₹|rs\.?)?\s*\d|"
        r"owner wants\s*(?:₹|rs\.?)?\s*\d|getting vacated|gets vacated|vacant|"
        r"ready to move|for showing|showing rent|call for visit|site visit|deal available)\b", n))

def predict_message(raw):
    raw=raw or ""
    p=v433.predict_message(raw)
    base=v432.predict_message(raw)
    rules=[p.get("rule","V433_BASE")]
    evidence=dict(p.get("evidence") or {})

    cls=p["class"]; tx=p["transaction"]; own=p["ownership"]; conf=float(p["confidence"])
    demand_first=_demand_first(raw)
    direct_rent=_direct_rent_requirement(raw)
    offer=_explicit_offer_anchors(raw)
    rf=v431._repair_features(raw)
    mixed,sale_headers,rent_headers=v432._mixed_parent(raw)

    # 1) Requirement class is granted only when the source grammatically starts as
    # demand, or the earlier stable student already classified it as requirement.
    # This prevents a stray "required/client wants" inside an availability ad from
    # flipping a real property into REQUIREMENT.
    if cls=="REQUIREMENT" and base["class"]=="PROPERTY_AVAILABILITY" and not demand_first:
        cls="PROPERTY_AVAILABILITY"
        tx=base["transaction"]
        own="OWNED"
        conf=max(conf,float(base["confidence"]))
        rules.append("V434_AVAILABILITY_OWNS_STRAY_DEMAND_WORDS")

    # 2) Conversely, a source whose first clause is unmistakably demand remains a
    # requirement even if the desired transaction contains "for rent".
    elif demand_first and cls!="NOISE" and not (
        base["class"]=="PROPERTY_AVAILABILITY" and offer and not direct_rent
    ):
        cls="REQUIREMENT"
        own="OWNED"
        conf=max(conf,99.0)
        rules.append("V434_DEMAND_FIRST_CLASS_OWNERSHIP")

    # 3) Mixed parent ontology remains final.
    if cls=="INVENTORY_GROUP" and mixed:
        tx="AMBIGUOUS"
        own="OWNED"
        conf=max(conf,99.2)
        rules.append("V434_MIXED_PARENT_FINAL_PRECEDENCE")

    # 4) Requirement transaction: direct syntactic relation only. Generic same-clause
    # occurrence of "rent/lease" is insufficient. This is intentionally stricter than 4.3.3.
    elif cls=="REQUIREMENT":
        strong_rent,strong_purchase,_=v433._strict_requirement_tx(raw)
        if strong_purchase or rf["explicit_purchase_requirement"]:
            tx="SALE"
            conf=max(conf,99.0)
            rules.append("V434_REQUIREMENT_EXPLICIT_PURCHASE")
        elif direct_rent:
            tx="RENT"
            conf=max(conf,99.0)
            rules.append("V434_REQUIREMENT_DIRECT_RENT_RELATION")
        elif rf["low_budget"] and rf["residential_spec"] and not rf["plural_capital"]:
            tx="RENT"
            conf=max(conf,98.8)
            rules.append("V434_REQUIREMENT_RESIDENTIAL_MONTHLY_BUDGET")
        elif rf["plural_capital"]:
            tx="SALE"
            conf=max(conf,99.0)
            rules.append("V434_REQUIREMENT_CAPITAL_BUDGET_SALE")
        else:
            tx="UNKNOWN"
            rules.append("V434_REQUIREMENT_STRICT_ABSTAIN")
    ...
    evidence["v434"]={
        ...
        "direct_rent_requirement":direct_rent,
        "explicit_offer_anchors":offer,
        "stable_base_class":base["class"],
        "stable_base_transaction":base["transaction"],
        "policy":"Demand-first grammatical ownership; stable-base availability protection; direct rent relation only."
    }
...
def training_status(engine):
    ...
    lessons=(list(v430.LESSON_REGRESSION)+list(v431.MASTER_REPAIR_REGRESSION)+
             list(v432.FINAL_REPAIR_REGRESSION)+list(v433.CLOSURE_REGRESSION)+CLOSURE_REGRESSION)
    ...
    return {"version":VERSION,"v4_training":v4_score,"legacy_regression":legacy_score,
            ...
            "training_gate":"V434_TRAINING_PASS_READY_FOR_FRESH_V5" if passed else "V434_TRAINING_HOLD_DO_NOT_FREEZE_V5",
            "scientific_policy":"V4 is closed training data only. Fresh V5 remains untouched until cumulative PASS.",
            "safety":{"production_writes":0,"whatsapp_writes":0,"gold_mutations":0}}

def _record_training(engine,s):
    with engine.begin() as conn:
        conn.execute(text("""INSERT INTO alliance_student_v434_runs
...
def _used_blind_ids(conn):
    ...
    for t in ["alliance_mastery_v340_blind_audit_cases","alliance_mastery_v360_exam_v2_cases",
              ...
              "alliance_v5_433_cases","alliance_v5_434_cases"]:
...
def _select_v5(pool):
    ...
    for x in remaining:
        ...
        selected.append(x)
    selected.sort(key=lambda x:int(hashlib.sha256(("V5_434_ORDER|"+str(x[0]["blind_id"])).encode()).hexdigest()[:12],16))
...
def freeze_v5(engine):
    ...
    if train["training_gate"]!="V434_TRAINING_PASS_READY_FOR_FRESH_V5":
        return {"status":"BLOCKED","reason":"4.3.4 training/regression gate HOLD.","training":train}
    with engine.connect() as conn:
        existing=conn.execute(text("SELECT COUNT(*) FROM alliance_v5_434_cases WHERE exam_version=:e"),{"e":V5_EXAM_VERSION}).scalar() or 0
        if existing:
            m=conn.execute(text("SELECT predictor_sha256,case_manifest_hash,status FROM alliance_v5_434_manifest WHERE exam_version=:e"),{"e":V5_EXAM_VERSION}).mappings().first()
    ...
    with engine.begin() as conn:
        conn.execute(text("""INSERT INTO alliance_v5_434_manifest
        ...
        {"id":str(uuid.uuid4()),"e":V5_EXAM_VERSION,"t":V5_TARGET,"pv":VERSION,"ps":psha,
         "policy":"Untouched frozen blind pool only; excludes all prior V1/V2/V3/V4/V5 IDs; no truth used for selection.","mh":mhash})
        for ordinal,(r,p,_,_) in enumerate(selected,1):
            conn.execute(text("""INSERT INTO alliance_v5_434_cases
...
def _adjudicate(engine):
    ...
    with engine.connect() as conn:
        rows=[dict(r) for r in conn.execute(text("SELECT * FROM alliance_v5_434_cases WHERE exam_version=:e ORDER BY ordinal"),{"e":V5_EXAM_VERSION}).mappings()]
    for r in rows:
        with engine.connect() as conn:
            if conn.execute(text("SELECT 1 FROM alliance_v5_434_truth WHERE audit_id=:id"),{"id":str(r["audit_id"])}).scalar(): continue
        ...
        with engine.begin() as conn:
            conn.execute(text("""INSERT INTO alliance_v5_434_truth
            ...
            {"id":str(uuid.uuid4()),"aid":str(r["audit_id"]),"e":V5_EXAM_VERSION,
             ...
             "con":_j({"status":"AUTO_RESOLVED" if ok else "EXCEPTION","fields":fields,
                       "policy":"Frozen pre-V5 examiner stack; Student 4.3.4 excluded."}),
...
def v5_report(engine):
    with engine.connect() as conn:
        ...
        FROM alliance_v5_434_cases c LEFT JOIN alliance_v5_434_truth t ON t.audit_id=c.audit_id
    ...
    result={"version":VERSION,"exam_version":V5_EXAM_VERSION,"total":len(rows),"auto_resolved":len(rows),"unresolved":0,
            ...
            "truth_policy":"Frozen 4.2.1/4.2.2/4.2.3/4.2.5 examiner stack; Student 4.3.4 excluded.",
            "safety":{"student_tuning_during_v5":0,"production_writes":0,"whatsapp_writes":0,"gold_mutations":0}}
    th=hashlib.sha256(json.dumps(payload,separators=(",",":")).encode()).hexdigest()
    with engine.begin() as conn:
        conn.execute(text("""INSERT INTO alliance_v5_434_results
    ...
    with engine.connect() as conn:
        return conn.execute(text("SELECT result FROM alliance_v5_434_results WHERE exam_version=:e"),{"e":V5_EXAM_VERSION}).scalar() or result
...
def _dashboard(engine):
    ...
    if s.get("status")=="TRAINING_HOLD":
        banner="<div class='warn'>4.3.4 cumulative training HOLD. Fresh V5 NOT frozen.</div>"
    elif ex.get("certification_gate")=="AUTOMATED_INDEPENDENT_V5_PASS":
        banner="<div class='ok'>✓ STUDENT 4.3.4 PASSED FRESH AUTOMATED V5</div>"
    elif ex.get("certification_gate")=="AUTOMATED_INDEPENDENT_V5_HOLD":
        banner="<div class='warn'>Fresh V5 complete: Student 4.3.4 HOLD. V5 remains immutable.</div>"
    else:
        banner=f"<div class='warn'>V5 automated truth remaining: {ex.get('unresolved','—')}. No manual work requested.</div>"
    return f"""<!doctype html><html><head><meta charset='utf-8'><title>Alliance Autonomous Student 4.3.4</title>
    ...
    <h1>Alliance Autonomous Student 4.3.4 → Fresh V5</h1>
    <p>Demand ownership closure: demand-first classification, stable availability protection, and direct rent-requirement relation only.</p>
    <div class='grid'>
...
def register(core):
    ...
    if not foundation._route_exists(app,"/api/property-brain/autonomous-v434/status"):
        @app.get("/api/property-brain/autonomous-v434/status")
        def status_v434(): return run(engine)
    if not foundation._route_exists(app,"/property-brain/autonomous-v434"):
        @app.get("/property-brain/autonomous-v434",response_class=HTMLResponse)
        def page_v434(): return HTMLResponse(_dashboard(engine))
    try: run(engine)
    except Exception: pass
    return {"status":"REGISTERED","version":VERSION,"route":"/property-brain/autonomous-v434",
            "policy":"CUMULATIVE_DEMAND_OWNERSHIP_CLOSURE_THEN_FRESH_V5",
