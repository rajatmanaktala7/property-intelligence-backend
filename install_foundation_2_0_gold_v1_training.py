...
MODULE = ROOT / "alliance_property_brain_gold_v2.py"
...
def main():
    if not FOUNDATION.exists():
        fail("alliance_property_brain_foundation_v1.py not found. Run inside repository root.")
    if not OPTIONAL.exists():
        fail("alliance_optional_property_modules.py not found. Run inside repository root.")
    ...
    if 'VERSION = "1.9.31-ALLIANCE-PROPERTY-BRAIN-FOUNDATION"' not in foundation_text:
        fail("Expected Foundation 1.9.31 baseline is not present. Nothing was changed.")
    missing = [m for m in REQUIRED_MARKERS if m not in foundation_text]
    if missing:
        fail("Prior Gold fixes are missing: " + ", ".join(missing) + ". Nothing was changed.")

    before_foundation = FOUNDATION.read_bytes()
    stamp = datetime.now().strftime("%Y%m%d-%H%M%S")
    backup = OPTIONAL.with_name(f"{OPTIONAL.name}.before-gold-v2-{stamp}.bak")
    shutil.copy2(OPTIONAL, backup)

    MODULE.write_text(MODULE_SOURCE, encoding="utf-8")

    marker = "# FOUNDATION 2.0 - Gold V1 freeze + tutor training + benchmark"
    if marker not in optional_text:
        needle = "\n    return result"
        pos = optional_text.rfind(needle)
        if pos < 0:
            fail("Could not find final return result in alliance_optional_property_modules.py.")
        optional_text = optional_text[:pos] + "\n" + REGISTRATION_BLOCK + optional_text[pos:]
        OPTIONAL.write_text(optional_text, encoding="utf-8")

    py_compile.compile(str(MODULE), doraise=True)
    py_compile.compile(str(OPTIONAL), doraise=True)
    py_compile.compile(str(FOUNDATION), doraise=True)

    if before_foundation != FOUNDATION.read_bytes():
        fail("Foundation changed unexpectedly. Restore from git before continuing.")
    ...
    if missing_after:
        fail("Regression check failed after install: " + ", ".join(missing_after))
    ...
    checks = [
        ...
        'SNAPSHOT_VERSION = "GOLD_V1_100"',
        'TRAINING_VERSION = "PROPERTY_BRAIN_GOLD_TUTOR_V1"',
        '"/property-brain/gold-v2"',
        '("CITY","GGN","Gurgaon"',
        '"production_write_permission":False',
    ]
    missing_module = [m for m in checks if m not in module_text]
    if missing_module:
        fail("New module validation failed: " + ", ".join(missing_module))

    print()
    print("SUCCESS: Foundation 2.0 Gold V1 training layer installed.")
    print("Foundation file unchanged:", FOUNDATION.name)
    print("New module:", MODULE.name)
    print("Patched registrar:", OPTIONAL.name)
    print("Backup:", backup.name)
    print("Preserved prior Gold fixes: YES")
    print("Production inventory writes enabled: NO")
    print("Dashboard after Railway deploy: /property-brain/gold-v2")
    print()
    print("Next after deploy:")
    print("1. Open /property-brain/gold-v2")
    print("2. Click Train From Gold V1")
    print("3. Share the benchmark result with ChatGPT")
