...
def _property_page(d):
    ...
    city='Goa' if d=='GOA' else 'Delhi NCR'
    return f"""<!doctype html><html><head><meta charset=utf-8><meta name=viewport content="width=device-width,initial-scale=1"><title>Fast Property Entry V19.3</title>
<style>
...
function render(kind){{const ids={{IMAGE:'ilist',VIDEO:'vlist',BROCHURE:'blist'}},a=selected[kind];document.getElementById(ids[kind]).textContent=a.length?a.length+' selected\\n'+a.map((x,i)=>(i+1)+'. '+x.name).join('\\n'):'No files selected.'}}
