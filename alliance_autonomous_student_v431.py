...
VERSION = "4.3.1-ALLIANCE-AUTONOMOUS-STUDENT-V5-MASTERY-REPAIR"
MODE = "CUMULATIVE_TRANSACTION_FOOTER_RENTAL_BUDGET_REPAIR_THEN_FRESH_V5"
RULESET_VERSION = "CRE_STUDENT_2026_09_03_V431"
V4_EXAM_VERSION = v410.EXAM_VERSION
V5_EXAM_VERSION = "BLIND_AUDIT_V5_431_2026_09_03"
V5_TARGET = 20
OVERALL_PASS = 95.0
FIELD_PASS = 90.0

DDL = [
"""CREATE TABLE IF NOT EXISTS alliance_student_v431_runs(
...
"""CREATE TABLE IF NOT EXISTS alliance_v5_431_manifest(
...
"""CREATE TABLE IF NOT EXISTS alliance_v5_431_cases(
...
"""CREATE TABLE IF NOT EXISTS alliance_v5_431_truth(
...
"""CREATE TABLE IF NOT EXISTS alliance_v5_431_results(
...
def _lines(raw): return v400._lines(raw or "")
...
# ---------------------------------------------------------------------------
# 4.3.1 mastery repair.
# Generic fixes only. No V4 source hash or exact-text lookup exists anywhere.
# ---------------------------------------------------------------------------

def _repair_features(raw):
    n = _norm(raw)
    lines = _lines(raw)

    plural_capital = bool(re.search(
        r"(?:₹|rs\.?\s*)?\d+(?:\.\d+)?\s*(?:cr|crores?)\b|"
        r"\b(?:budget|demand|asking|price)\b.{0,45}(?:₹|rs\.?)?\s*\d+(?:\.\d+)?\s*(?:cr|crores?)\b", n))

    low_budget = bool(re.search(
        r"\bbudget\b.{0,35}(?:₹|rs\.?)?\s*\d+(?:\.\d+)?\s*(?:k|thousand)\b|"
        r"\bbudget\b.{0,35}(?:₹|rs\.?)?\s*(?:0?\.)?\d+(?:\.\d+)?\s*(?:lac|lakh|lacs|lakhs)\b", n))

    residential_spec = bool(re.search(
        r"\b(?:\d+(?:\.\d+)?\s*bhk|flat|apartment|villa|house|furnished|semi furnished|"
        r"unfurnished|family|staff accommodation)\b", n))

    req = bool(re.search(
        r"\b(?:require|requires|required|requirement|wanted|need(?:ed)?|seeking|looking for|"
        r"client wants?|buyer required|tenant requirement|wants? to purchase|want to purchase)\b", n))

    explicit_rental_requirement = bool(
        re.search(r"\b(?:require|requires|required|requirement|wanted|need(?:ed)?|looking for)\b.{0,100}\b(?:rent|rental|lease)\b", n)
        or re.search(r"\b(?:rent|rental|lease)\b.{0,100}\b(?:require|requires|required|requirement|wanted|need(?:ed)?)\b", n)
        or re.search(r"\btenant requirement\b", n)
    )

    explicit_purchase_requirement = bool(
        re.search(r"\b(?:wants? to purchase|want to purchase|looking to buy|looking to purchase|buyer required|purchase requirement)\b", n)
        or (req and plural_capital and not explicit_rental_requirement)
    )

    already_rented = bool(re.search(
        r"\b(?:already rented|currently rented|rented to|pre[- ]?rented|pre[- ]?leased|"
        r"freshly leased|leased to|rental income|roi)\b", n))

    explicit_sale = bool(re.search(
        r"\b(?:for sale|available for sale|sale inventory|resale|outright|asking price|"
        r"owner wants to sell|sell(?:ing)?)\b", n))

    explicit_rent_offer = bool(re.search(
        r"\b(?:for rent|available for rent|available for lease|available on lease|"
        r"to[- ]?let|asking rent|getting vacated|showing rent)\b", n))

    many_more_footer = bool(re.search(
        r"\bmany more (?:options|properties|units) available\b|"
        r"\bmore options available\b", n))

    concrete_multi = (
        len(re.findall(r"(?:₹|rs\.?\s*)?\d+(?:\.\d+)?\s*(?:cr|crore|crores|lac|lakh|lacs|lakhs|l)\b", n)) >= 2
        and (
            len(re.findall(r"\b\d+(?:\.\d+)?\s*bhk\b", n)) >= 2
            or len(re.findall(r"\b(?:upper ground|ug|ground|first|second|third|top|terrace|basement)\b", n)) >= 2
            or len(re.findall(r"\b(?:dlf|m3m|emaar|aipl|ireo|bestech|tulip|elan|ats|unitech|mahindra|vipul|suncity|sobha|godrej|krisumi|raheja)\b", n)) >= 3
        )
    )

    return {
        "plural_capital": plural_capital,
        "low_budget": low_budget,
        "residential_spec": residential_spec,
        "req": req,
        "explicit_rental_requirement": explicit_rental_requirement,
        "explicit_purchase_requirement": explicit_purchase_requirement,
        "already_rented": already_rented,
        "explicit_sale": explicit_sale,
        "explicit_rent_offer": explicit_rent_offer,
        "many_more_footer": many_more_footer,
        "concrete_multi": concrete_multi,
    }

def predict_message(raw):
    raw = raw or ""
    p = v430.predict_message(raw)
    n = _norm(raw)
    f = _repair_features(raw)
    rules = [p.get("rule","V430_BASE")]
    ...
    # 1. A generic footer saying "many more options available" does not create
    # a multi-property entity when the message contains only one concrete property.
    if cls == "INVENTORY_GROUP" and f["many_more_footer"] and not f["concrete_multi"]:
        base = v402.predict_message(raw)
        if base["class"] == "PROPERTY_AVAILABILITY" or f["explicit_rent_offer"] or f["explicit_sale"]:
            cls = "PROPERTY_AVAILABILITY"
            own = "OWNED"
            conf = max(conf, 99.0)
            rules.append("V431_GENERIC_FOOTER_NOT_INVENTORY_GROUP")

    # 2. Requirement transaction semantics.
    if cls == "REQUIREMENT":
        if f["explicit_purchase_requirement"]:
            tx = "SALE"
            conf = max(conf, 99.0)
            rules.append("V431_REQUIREMENT_PURCHASE_OR_CAPITAL_SALE")
        elif f["explicit_rental_requirement"] or (f["low_budget"] and f["residential_spec"]):
            tx = "RENT"
            conf = max(conf, 98.8)
            rules.append("V431_REQUIREMENT_LOW_MONTHLY_BUDGET_RENT")
        else:
            # Do not convert a requirement to RENT just because an unrelated
            # occurrence of the word rent/lease appears somewhere in the message.
            if not f["explicit_purchase_requirement"]:
                tx = "UNKNOWN"
                rules.append("V431_REQUIREMENT_TRANSACTION_RELATION_GUARD")

    # 3. Pre-rented / already-rented investment asset with capital consideration is SALE.
    if cls in {"PROPERTY_AVAILABILITY","INVENTORY_GROUP"} and f["already_rented"] and f["plural_capital"]:
        tx = "SALE"
        own = "OWNED"
        conf = max(conf, 99.2)
        rules.append("V431_ALREADY_RENTED_CAPITAL_IS_SALE")

    # 4. Explicit sale overrides rent-as-occupancy when capital sale evidence exists.
    elif cls in {"PROPERTY_AVAILABILITY","INVENTORY_GROUP"} and f["explicit_sale"] and f["plural_capital"]:
        tx = "SALE"
        conf = max(conf, 99.0)
        rules.append("V431_EXPLICIT_SALE_WITH_RENT_OCCUPANCY")

    # 5. Plural crore normalization, e.g. "180 Crores".
    elif cls == "REQUIREMENT" and f["plural_capital"] and not f["explicit_rental_requirement"]:
        tx = "SALE"
        conf = max(conf, 99.0)
        rules.append("V431_CRORE_PLURAL_CAPITAL_NORMALIZATION")
    ...
    evidence["v431_repairs"] = f
    evidence["v431_policy"] = "Generic semantic repairs only; no V4 text/hash memorization."
    return {
        "class":cls,
        "transaction":tx,
        "ownership":own,
...
# Additional anti-overfit variants for exact failure families.
MASTER_REPAIR_REGRESSION = [
    ("requirement_low_budget_variant",
     "Require 2 BHK furnished in Panjim for company staff. Budget 45k.",
     "REQUIREMENT","RENT","OWNED"),
    ("requirement_unknown_guard_variant",
     "Requirement house 250 sq yd in Sushant Lok. Direct broker contact.",
     "REQUIREMENT","UNKNOWN","OWNED"),
    ("crores_plural_requirement_variant",
     "Immediate required large freehold plot in South Delhi. Client Budget Rs 125 Crores.",
     "REQUIREMENT","SALE","OWNED"),
    ("already_rented_sale_variant",
     "Already rented commercial unit. Rent 95k monthly. Demand 3.75 Cr.",
     "PROPERTY_AVAILABILITY","SALE","OWNED"),
    ("generic_footer_single_variant",
     "Defence Colony 3 BHK floor available for rent 1.8L. Many more options available.",
     "PROPERTY_AVAILABILITY","RENT","OWNED"),
    ("true_multi_group_control",
...
def _v4_truth_rows(engine):
    # Reuse the already completed V4 independent truth reader from 4.3.0.
    return v430._v4_truth_rows(engine)
...
def training_status(engine):
    ...
    lessons=[]
    for suite in (v430.LESSON_REGRESSION,MASTER_REPAIR_REGRESSION):
        lessons.extend(suite)
    ...
    return {
        ...
        "training_gate":"V431_TRAINING_PASS_READY_FOR_FRESH_V5" if passed else "V431_TRAINING_HOLD_DO_NOT_FREEZE_V5",
        "scientific_policy":"V4 closed training only. V5 uses untouched blind IDs and frozen pre-V5 examiner stack.",
        "safety":{"production_writes":0,"whatsapp_writes":0,"gold_mutations":0},
    }

def _record_training(engine,s):
    with engine.begin() as conn:
        conn.execute(text("""INSERT INTO alliance_student_v431_runs
        ...
        {"id":str(uuid.uuid4()),"rv":RULESET_VERSION,
         "va":s["v4_training"]["accuracy"],"vc":s["v4_training"]["field_accuracy"]["class"],
         "vt":s["v4_training"]["field_accuracy"]["transaction"],"vo":s["v4_training"]["field_accuracy"]["ownership"],
         "la":s["legacy_regression"]["accuracy"],"lr":s["lesson_regression"]["accuracy"],
         "gate":s["training_gate"],"res":_j(s)})
...
def _candidate_pool(engine):
    with engine.connect() as conn:
        rows=[dict(r) for r in conn.execute(text("""
          ...
          WHERE status='FROZEN' ORDER BY blind_id
        """)).mappings()]
...
def _select_v5(pool):
    ...
    for x in remaining:
        ...
        selected.append(x)
    selected.sort(key=lambda x:int(hashlib.sha256(("V5_431_ORDER|"+str(x[0]["blind_id"])).encode()).hexdigest()[:12],16))
...
def freeze_v5(engine):
    ...
    if train["training_gate"]!="V431_TRAINING_PASS_READY_FOR_FRESH_V5":
        return {"status":"BLOCKED","reason":"4.3.1 training/regression gate HOLD.","training":train}
    with engine.connect() as conn:
        existing=conn.execute(text("SELECT COUNT(*) FROM alliance_v5_431_cases WHERE exam_version=:e"),{"e":V5_EXAM_VERSION}).scalar() or 0
        if existing:
            manifest=conn.execute(text("SELECT predictor_sha256,case_manifest_hash,status FROM alliance_v5_431_manifest WHERE exam_version=:e"),{"e":V5_EXAM_VERSION}).mappings().first()
    ...
    with engine.begin() as conn:
        conn.execute(text("""INSERT INTO alliance_v5_431_manifest
        ...
        {"id":str(uuid.uuid4()),"e":V5_EXAM_VERSION,"t":V5_TARGET,"pv":VERSION,"ps":psha,
         "policy":"Untouched frozen blind pool only; excludes V1/V2/V3/V4 and any earlier V5 IDs; no truth used for selection.",
         "mh":mhash})
        for ordinal,(r,p,_,_) in enumerate(selected,1):
            conn.execute(text("""INSERT INTO alliance_v5_431_cases
            ...
            {"id":str(uuid.uuid4()),"bid":str(r["blind_id"]),"e":V5_EXAM_VERSION,"ord":ordinal,"sh":r["source_hash"],
             "raw":r["raw_text"],"cl":p["class"],"tx":p["transaction"],"ow":p["ownership"],
             "cf":float(p["confidence"]),"rule":p.get("rule")})
    return {"status":"FROZEN","total":V5_TARGET,"predictor_sha256":psha,"case_manifest_hash":mhash}

def _exam_judges(engine,raw):
    # Frozen pre-V5 truth machinery only. Student 4.3.1 is excluded.
    out={}
...
def _resolve_field(judges,field):
    conf_key=f"{field}_confidence"; votes=[]
    for name,j in judges.items():
        val=j.get(field); cf=float(j.get(conf_key) or 0)
    ...
    accepted=((len(wvotes)>=4 and avg>=0.96 and len(strong_dissent)<=1)
              or (len(wvotes)>=3 and avg>=0.975 and core_winners>=2 and not strong_dissent))
    if not accepted:
...
def _adjudicate(engine):
    ...
    with engine.connect() as conn:
        rows=[dict(r) for r in conn.execute(text("SELECT * FROM alliance_v5_431_cases WHERE exam_version=:e ORDER BY ordinal"),{"e":V5_EXAM_VERSION}).mappings()]
    for r in rows:
        with engine.connect() as conn:
            if conn.execute(text("SELECT 1 FROM alliance_v5_431_truth WHERE audit_id=:id"),{"id":str(r["audit_id"])}).scalar(): continue
        ...
        with engine.begin() as conn:
            conn.execute(text("""INSERT INTO alliance_v5_431_truth
            ...
            {"id":str(uuid.uuid4()),"aid":str(r["audit_id"]),"e":V5_EXAM_VERSION,
             ...
             "con":_j({"status":"AUTO_RESOLVED" if ok else "EXCEPTION","fields":fields,
                       "policy":"Frozen pre-V5 examiner stack; Student 4.3.1 excluded."}),
...
def v5_report(engine):
    with engine.connect() as conn:
        ...
        FROM alliance_v5_431_cases c
        LEFT JOIN alliance_v5_431_truth t ON t.audit_id=c.audit_id
    ...
    fs={f:[0,0] for f in ("class","transaction","ownership")}; errors=[]; caseok=0; truth_payload=[]
    for r in rows:
    ...
    result={"version":VERSION,"exam_version":V5_EXAM_VERSION,"total":len(rows),"auto_resolved":len(rows),
            ...
            "truth_policy":"Frozen 4.2.1/4.2.2/4.2.3/4.2.5 examiner stack; Student 4.3.1 excluded.",
            "safety":{"student_tuning_during_v5":0,"production_writes":0,"whatsapp_writes":0,"gold_mutations":0}}
    th=hashlib.sha256(json.dumps(truth_payload,separators=(",",":")).encode()).hexdigest()
    with engine.begin() as conn:
        conn.execute(text("""INSERT INTO alliance_v5_431_results
    ...
    with engine.connect() as conn:
        return conn.execute(text("SELECT result FROM alliance_v5_431_results WHERE exam_version=:e"),{"e":V5_EXAM_VERSION}).scalar() or result
...
def run(engine):
    ...
    if train["training_gate"]!="V431_TRAINING_PASS_READY_FOR_FRESH_V5":
        return {"version":VERSION,"status":"TRAINING_HOLD","training":train,"v5":{"status":"NOT_FROZEN"},
                "next_step":"Automation must continue repairing only failed training/regression concepts before V5.",
                "safety":{"production_writes":0,"whatsapp_writes":0,"gold_mutations":0}}
...
def _dashboard(engine):
    ...
    if s.get("status")=="TRAINING_HOLD":
        banner="<div class='warn'>4.3.1 training/regression HOLD. Fresh V5 NOT frozen.</div>"
    elif ex.get("certification_gate")=="AUTOMATED_INDEPENDENT_V5_PASS":
        banner="<div class='ok'>✓ STUDENT 4.3.1 PASSED FRESH V5</div>"
    elif ex.get("certification_gate")=="AUTOMATED_INDEPENDENT_V5_HOLD":
        banner="<div class='warn'>Fresh V5 complete: Student 4.3.1 HOLD.</div>"
    else:
        banner=f"<div class='warn'>V5 automated truth remaining: {ex.get('unresolved','—')}. No manual work requested.</div>"
    return f"""<!doctype html><html><head><meta charset='utf-8'><title>Alliance Autonomous Student 4.3.1</title>
    ...
    <h1>Alliance Autonomous Student 4.3.1 → Fresh V5</h1>
    <p>Repairs only the concepts exposed by the 4.3 training HOLD. V5 remains protected until every regression gate passes.</p>
    <div class='grid'>
...
def register(core):
    ...
    if not foundation._route_exists(app,"/api/property-brain/autonomous-v431/status"):
        @app.get("/api/property-brain/autonomous-v431/status")
        def status_v431(): return run(engine)
    if not foundation._route_exists(app,"/property-brain/autonomous-v431"):
        @app.get("/property-brain/autonomous-v431",response_class=HTMLResponse)
        def page_v431(): return HTMLResponse(_dashboard(engine))
    try: run(engine)
    except Exception: pass
    return {"status":"REGISTERED","version":VERSION,"route":"/property-brain/autonomous-v431",
            "policy":"CUMULATIVE_MASTERY_REPAIR_THEN_FRESH_V5",
