...
MODULE_VERSION = "2.4.7-ENTITY-DUPLICATE-RESOLUTION"
BATCH_SIZE = 250
...
def derive_entity_identity(row, raw_text):
    ...
    budget = _round_money(row.get("recovered_budget"))
    ...
    exact_fp = _sha(entity_kind, tx, loc, ptype, amin, amax, budget, floor, suitable, raw_clean)
    ...
    # Canonical entity key deliberately excludes raw message text and poster identity.
    ...
    try:
        ...
        if not ids:
            return {
                ...
                "next_step": "Run V2.5 production matcher",
            }
            ...
            purity_rows = {
                ...
                for r in c.execute(text(f"""
                  SELECT listing_id,recovered_role,recovered_transaction,recovered_location,
                         ...
                         recovered_budget,recovered_required_floor,recovered_suitable_for,purity_score
        ...
        # Existing canonical representatives from prior batches.
        with primary_engine.connect() as c:
            existing = c.execute(text("""
              SELECT listing_id,canonical_entity_key,exact_fingerprint,fuzzy_fingerprint,
                     phone_fingerprint,entity_kind
        ...
        for rid in ids:
            ...
            elif ident["phone_fingerprint"] and ident["phone_fingerprint"] in by_phone:
            ...
            # Register representative in lookup maps for following rows in same batch.
        ...
        return {
            ...
            "likely_duplicates": stats["likely"],
            ...
            "source_data_modified": False,
            "next_step": "Run V2.5 production matcher" if int(remaining) == 0 else "Run next entity-resolution batch",
        }
    finally:
        if owned:
    ...
    with engine.connect() as c:
        r = c.execute(text("""
          SELECT
            ...
            COUNT(*) FILTER (WHERE duplicate_type='LIKELY_DUPLICATE')::int likely_duplicates,
...
def register_entity_resolution_routes(core):
    ...
    @app.post("/api/v2/intelligence/whatsapp-entities/run")
    def run(req: Request):
        if hasattr(core, "need_login"):
            core.need_login(req)
        return run_entity_resolution(engine)

    @app.get("/api/v2/intelligence/whatsapp-entities/summary")
    def get_summary(req: Request):
        if hasattr(core, "need_login"):
            core.need_login(req)
        return {"version": MODULE_VERSION, **entity_summary(engine)}

    @app.get("/v2/whatsapp-entity-resolution", response_class=HTMLResponse)
    def page(req: Request):
        if hasattr(core, "need_login"):
            core.need_login(req)
        s = entity_summary(engine)
        return HTMLResponse(f"""<!doctype html>
<html><head><meta charset="utf-8"><title>Entity & Duplicate Resolution</title></head>
...
<h1>V2.4.7 Entity & Duplicate Resolution</h1>
...
<p>Likely duplicates: <b>{s.get('likely_duplicates',0)}</b></p>
<p>Suppressed from matcher: <b>{s.get('suppressed',0)}</b></p>
<p>All raw source records are preserved.</p>
</div></body></html>""")
