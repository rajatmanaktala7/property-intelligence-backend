...
def _sync_live_property(engine, row: Dict[str, Any], normalized: Dict[str, Any], source_hash: str):
    ...
    clean = {
        ...
        "original_message": normalized.get("raw_text"),
        "raw_message": normalized.get("raw_text"),
        "transaction_type": tx,
        "location": normalized.get("locality"),
        "locality": normalized.get("locality"),
        "family": normalized.get("family"),
        "subtype": normalized.get("subtype"),
        "property_type": _norm(row.get("property_type")) or normalized.get("subtype") or normalized.get("family"),
        "configuration_details": _norm(row.get("property_type")) or normalized.get("subtype"),
        "area_sqft": normalized.get("area_sqft"),
        "area_text": _norm(row.get("available_area_sqft") or row.get("area_sqft")),
        "rent_inr": normalized.get("rent_inr"),
        "sale_price_inr": normalized.get("sale_price_inr"),
        "sender_name": _norm(row.get("sender_name")),
        "sender_phone": _norm(row.get("sender_phone")),
        "broker_name": _norm(row.get("broker_name")),
        "broker_phone": _norm(row.get("broker_phone")),
        "owner_name": _norm(row.get("owner_name")),
        "owner_phone": _norm(row.get("owner_phone")),
        "phones": normalized.get("phones") or [],
        "source": _norm(row.get("source_id")) or "WhatsApp Live",
        "wa_property_id": sid,
    }
