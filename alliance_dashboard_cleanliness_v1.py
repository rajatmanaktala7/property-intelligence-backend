...
def extract_prices(raw):
    ...
    sale_patterns = [
        r"(?:for\s+sale|sale\s+price|sale\s+demand|selling\s+price|selling|sale)"
        r"\s*[-:=@]?\s*" + PRICE,
        PRICE + r"\s*(?:for\s+sale|sale)"
    ]

    rent_patterns = [
        r"(?:for\s+rent|asking\s+rent|monthly\s+rent|rent|lease)"
        r"\s*[-:=@]?\s*" + PRICE,
...
def quality_status(row):
    raw = norm(
        row.get("raw_property_text")
        or row.get("raw_message")
        or row.get("message_text")
        or row.get("raw_text")
    )
    ...
    if (
        any(x in low for x in requirement_words)
        and not any(x in low for x in availability_words)
    ):
...
def ensure_columns(engine):
    statements = [
        "ALTER TABLE v81_wa_clean_properties "
        "ADD COLUMN IF NOT EXISTS sale_price_inr NUMERIC(16,2)",

        "ALTER TABLE v81_wa_clean_properties "
        "ADD COLUMN IF NOT EXISTS rent_inr NUMERIC(16,2)",

        "ALTER TABLE v81_wa_clean_properties "
        "ADD COLUMN IF NOT EXISTS rent_per_sqft NUMERIC(14,2)",
        ...
        "ALTER TABLE v81_wa_clean_properties "
        "ADD COLUMN IF NOT EXISTS dashboard_exclusion_reasons "
        "JSONB DEFAULT '[]'::jsonb"
    ]
...
def harden_clean_database(engine):
    ...
    with engine.begin() as conn:
        ...
        for source in rows:
            ...
            property_type = repair_property_type(
                raw,
            ...
            if "price_inr" in columns:
                sets.append(
                    "price_inr=COALESCE(:primary_price,price_inr)"
                )

                params["primary_price"] = (
                    sale if sale is not None else rent
                )

            if "price_display" in columns:
                sets.append(
                    "price_display="
                    "COALESCE(NULLIF(:price_display,''),price_display)"
                )
            ...
            sql = (
                "UPDATE v81_wa_clean_properties SET "
                + ", ".join(sets)
                + " WHERE id=:id"
            )
...
def install_patch():
    ...
    engine = getattr(capture, "engine", None)

    if engine is not None:
        return harden_clean_database(engine)
