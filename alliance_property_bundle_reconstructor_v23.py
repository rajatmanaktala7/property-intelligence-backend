...
VERSION = "2.3.1-PROPERTY-BUNDLE-RECONSTRUCTOR-PREVIEW"
...
def register(core):
        ...
        return JSONResponse(
            {
                ...
                "fixes": [
                    "UNIT_ANCHOR_PAIRING",
                    "BLOCK_ANCHOR_PAIRING",
                    "LOCALITY_ANCHOR_PAIRING",
                    "REQUIREMENT_CONTINUATION_MERGER",
                    "SAFER_CONTEXT_INHERITANCE",
                    "DENSE_MULTI_ENTITY_HOLD",
                ],
                "evidence_model": [
                    "OWN_FACT",
                    "INHERITED_CONTEXT",
                    "SIBLING_FACT_DO_NOT_COPY",
                ],
    ...
    def preview(limit: int = Query(25, ge=1, le=100)):
        ...
        inherited_context_entities = 0
        sibling_isolated_entities = 0
        method_counts = {}
        ...
        for row in rows:
            entities = reconstruct_entities(
                row.get("burst_text") or ""
            )

            old_count = int(
                row.get("old_segment_count") or 0
            )
            ...
            for entity in entities:
                ...
                method_counts[entity.method] = (
                    method_counts.get(entity.method, 0) + 1
                )

                payload.append(
                    _safe_entity_payload(entity)
                )
            ...
            bursts.append(
                {
                    ...
                    "open_review_count": int(
                        row.get("open_review_count") or 0
                    ),
        ...
        return JSONResponse(
            {
                ...
                "writes_performed": 0,
                "decision": (
                    "SHADOW ONLY. Phase 2.3.1 fixes under-splitting, "
                    "over-splitting and unsafe context inheritance. "
                    "Do not switch the live segmenter until benchmark approval."
                ),
                "bursts": bursts,
            }
        )
