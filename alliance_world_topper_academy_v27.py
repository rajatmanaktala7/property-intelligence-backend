...
VERSION="2.7.1-WORLD-TOPPER-MAGIC-ACADEMY-HOTFIX"
MODE="EXAM_FIRST_ERROR_MEMORY_NO_BLIND_PROMOTION_SAFE_SCHEMA"
EXAM_VERSION="ALLIANCE_WORLD_TOPPER_EXAM_V1_1"

EXAM_DDL="""CREATE TABLE IF NOT EXISTS alliance_topper_exam_v27(
...
exam_score NUMERIC(6,2),precision_score NUMERIC(6,2),hallucination_penalty NUMERIC(6,2),
grade TEXT,decision TEXT,exam_version TEXT NOT NULL,created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
updated_at TIMESTAMPTZ NOT NULL DEFAULT now())"""

MASTERY_DDL="""CREATE TABLE IF NOT EXISTS alliance_topper_mastery_v27(
skill TEXT PRIMARY KEY,cases INTEGER NOT NULL DEFAULT 0,correct_count INTEGER NOT NULL DEFAULT 0,
wrong_count INTEGER NOT NULL DEFAULT 0,review_count INTEGER NOT NULL DEFAULT 0,
skill_precision NUMERIC(6,2),updated_at TIMESTAMPTZ NOT NULL DEFAULT now())"""
...
def _install(e):
    with e.begin() as c:
        c.execute(text(EXAM_DDL))
        c.execute(text(MASTERY_DDL))
        # Safe migration from any partially-created 2.7.0 mastery table.
        c.execute(text("ALTER TABLE alliance_topper_mastery_v27 ADD COLUMN IF NOT EXISTS correct_count INTEGER NOT NULL DEFAULT 0"))
        c.execute(text("ALTER TABLE alliance_topper_mastery_v27 ADD COLUMN IF NOT EXISTS wrong_count INTEGER NOT NULL DEFAULT 0"))
        c.execute(text("ALTER TABLE alliance_topper_mastery_v27 ADD COLUMN IF NOT EXISTS review_count INTEGER NOT NULL DEFAULT 0"))
        c.execute(text("ALTER TABLE alliance_topper_mastery_v27 ADD COLUMN IF NOT EXISTS skill_precision NUMERIC(6,2)"))
        # Existing exam table may already exist. Ensure all required columns safely.
        c.execute(text("ALTER TABLE alliance_topper_exam_v27 ADD COLUMN IF NOT EXISTS precision_score NUMERIC(6,2)"))
        c.execute(text("ALTER TABLE alliance_topper_exam_v27 ADD COLUMN IF NOT EXISTS hallucination_penalty NUMERIC(6,2)"))
        c.execute(text("ALTER TABLE alliance_topper_exam_v27 ADD COLUMN IF NOT EXISTS error_memory JSONB NOT NULL DEFAULT '[]'::jsonb"))
        c.execute(text("ALTER TABLE alliance_topper_exam_v27 ADD COLUMN IF NOT EXISTS field_marks JSONB NOT NULL DEFAULT '{}'::jsonb"))
...
def _exam_case(row):
    ...
    occupied=bool(re.search(r"\b(already\s+rented|rented\s+out|tenanted|rental\s+income|income\s+rent|leased\s+out)\b",low))
    ...
    for c in conflicts:
        if isinstance(c,dict) and c.get("field") in ("transaction_type","source_class"):
            errors.append("LIVE_CONFLICT_"+str(c.get("field")).upper())

    scored=[v for v in marks.values() if v!="MISSING"]
    correct=sum(v in ("PASS","CORRECTED") for v in scored)
    review=sum(v=="REVIEW" for v in scored)
    wrong=sum(v in ("FAIL","EVIDENCE_ONLY") for v in scored)
    ...
    grade="A+" if score>=95 and review==0 and wrong==0 else "A" if score>=90 else "B" if score>=80 else "C" if score>=70 else "D" if score>=55 else "F"
    decision="GRADUATE_CANDIDATE" if grade in ("A+","A") and not errors and wrong==0 else "TEACH_AND_RETEST"
    return student,answer,marks,sorted(set(errors)),score,precision,penalty,grade,decision

def _rebuild_mastery(e):
    with e.connect() as c:
        rows=c.execute(text("SELECT field_marks FROM alliance_topper_exam_v27 WHERE exam_version=:v"),{"v":EXAM_VERSION}).all()
    ...
    with e.begin() as c:
        c.execute(text("DELETE FROM alliance_topper_mastery_v27"))
        for f,a in agg.items():
            p=round(100*a["correct"]/max(a["cases"],1),2)
            c.execute(text("""INSERT INTO alliance_topper_mastery_v27
            (skill,cases,correct_count,wrong_count,review_count,skill_precision)
            VALUES(:f,:n,:ok,:w,:r,:p)
            ON CONFLICT(skill) DO UPDATE SET cases=EXCLUDED.cases,correct_count=EXCLUDED.correct_count,
            wrong_count=EXCLUDED.wrong_count,review_count=EXCLUDED.review_count,
            skill_precision=EXCLUDED.skill_precision,updated_at=now()"""),
...
def run(e,limit=1000):
    ...
    for r in rows:
        try:
            ...
            with e.begin() as c:
                c.execute(text("""INSERT INTO alliance_topper_exam_v27
            ...
            if errors and len(samples)<20:
                samples.append({"entity_id":r["entity_id"],"grade":grade,"score":score,"decision":decision,"errors":errors,"exam_answer":answer})
        except Exception as x:
            failed.append(f"{r.get('entity_id')}:{type(x).__name__}:{x}"[:500])
...
def status(e):
    _install(e)
    with e.connect() as c:
        s=c.execute(text("""SELECT count(*) n,avg(exam_score) score,avg(precision_score) precision_score_avg,
        ...
        mastery=[dict(x) for x in c.execute(text("""SELECT skill,cases,correct_count,wrong_count,review_count,skill_precision,updated_at
        FROM alliance_topper_mastery_v27 ORDER BY skill_precision ASC NULLS FIRST,cases DESC""")).mappings().all()]
        teach=[dict(x) for x in c.execute(text("""SELECT entity_id,grade,exam_score,precision_score,hallucination_penalty,
        decision,error_memory,exam_answer FROM alliance_topper_exam_v27
        WHERE exam_version=:v AND decision='TEACH_AND_RETEST'
    ...
    return foundation._json_safe({"status":"PASS","version":VERSION,"mode":MODE,"exam_version":EXAM_VERSION,
        ...
        "answer_precision":round(float(s["precision_score_avg"] or 0),2),
        "average_hallucination_penalty":round(float(s["penalty"] or 0),2),
        ...
        "hotfixes":["schema-safe mastery columns","safe partial-table migration","JSONB native-object loader","EVIDENCE_ONLY counts as not-correct"],
...
<h1>🪄 World Topper Magic Academy 2.7.1</h1><p>Gold discipline + atomic ownership + live WhatsApp exams + error memory.</p>
...
<button onclick='st()' style='padding:14px 20px'>Refresh</button><h2>Academy Scoreboard</h2><pre id=s></pre><h2>Exam Result</h2><pre id=r>No exam run yet.</pre>
