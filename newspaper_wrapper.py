...
ALLIANCE_V2_STATUS={"status":"LOADING_BACKGROUND","error":None}
...
V62_ERROR=None
V62_STATUS=None
LEGACY_BOOTSTRAP={"status":"LOADING_BACKGROUND","error":None,"started_at":datetime.now(timezone.utc).isoformat(),"completed_at":None}

def _legacy_bootstrap():
    global ALLIANCE_V2_STATUS, OPTIONAL_MODULES
    global V383_ERROR,V46_ERROR,V451_ERROR,V60_ERROR,V60_STATUS,V62_ERROR,V62_STATUS,LEGACY_BOOTSTRAP
    try:
        try:
        ...
        LEGACY_BOOTSTRAP={"status":"COMPLETE","error":None,"started_at":LEGACY_BOOTSTRAP.get("started_at"),"completed_at":datetime.now(timezone.utc).isoformat()}
    except Exception as e:
        LEGACY_BOOTSTRAP={"status":"DEGRADED","error":f"{type(e).__name__}: {e}","started_at":LEGACY_BOOTSTRAP.get("started_at"),"completed_at":datetime.now(timezone.utc).isoformat()}

threading.Thread(target=_legacy_bootstrap,daemon=True,name="alliance-legacy-wrapper-bootstrap").start()

@app.get("/production-health")
def production_health():
    return {
      "status":"OK","service":"Alliance Property Intelligence","wrapper":"V3.8+V6.2-UI-QUALITY",
...
def live_bootstrap_status():
    ...
    for r in app.router.routes:
        ...
        if p in {"/whatsapp-live","/whatsapp-live/feed","/api/v451/live/status","/api/v451/live/properties",
            ...
            paths.append(p)
    return {
      "status":"OK" if all(x is None for x in [V451_ERROR,V60_ERROR,V62_ERROR]) else "DEGRADED",
