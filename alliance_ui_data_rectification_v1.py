...
VERSION = "1.3.1-MANUAL-EDIT-WHATSAPP-SENDER-RECOVERY"
...
def _clean_location(v):
    s=re.sub(r"\s+"," ",str(v or "")).strip();low=s.lower();bad=("royal construction","royal constructions","construction pvt","constructions pvt","builders pvt","developers pvt","construction ltd","builders ltd","developers ltd")
...
def _master_properties(engine,q=""):
    ...
    trs=[]
    for raw in rows:
        d=_dict(raw);cr=_dict(d.get("clean_record"));loc=_clean_location(d.get("locality") or d.get("city") or _first(cr,["location","locality","city"]));phone=_phones(d.get("phones"),cr) or "Not captured";desc=_first(cr,["description","original_message","raw_text","configuration_details"]) or "Not captured";vals=[d.get("master_property_id") or d.get("canonical_id"),d.get("source_type"),loc,d.get("transaction_type"),d.get("area_sqft") or _first(cr,["area","area_sqft"]),d.get("price_raw") or _first(cr,["price","rent","sale_amount"]),desc,phone,d.get("promotion_status")]
    ...
    return _page("Master Properties",f"<div class=card><form><input name=q value='{_e(q)}' placeholder='Search Master Properties'> <button>Search</button></form><b>{len(rows)}</b> records shown</div><div class=tablebox><table><thead><tr>{''.join('<th>'+h+'</th>' for h in heads)}</tr></thead><tbody>{''.join(trs)}</tbody></table></div>")
...
def _wa_lookup(row):
    """Recover sender from the strongest available WhatsApp evidence chain."""
    direct=_phones(row)
    ...
    try:
        ...
        mid=str(_first(row,["message_id","wa_message_id","external_message_id"]) or "");rid=str(_first(row,["wa_requirement_id","source_pk","requirement_id","id"]) or "");raw=str(_first(row,["original_message","raw_text","message","description"]) or "").strip()
        with we.connect() as c:
            req=None
            if rid:
                try:req=c.execute(text("SELECT to_jsonb(x) FROM wa_requirements x WHERE CAST(wa_requirement_id AS TEXT)=:r OR CAST(id AS TEXT)=:r LIMIT 1"),{"r":rid}).scalar()
                except Exception:req=None
            if isinstance(req,dict):
                p=_phones(req.get("contact_phone"),req.get("sender_phone"),req)
                if p:return p
                mid=mid or str(req.get("message_id") or "")
            if mid:
                try:m=c.execute(text("SELECT to_jsonb(x) FROM wa_messages x WHERE CAST(message_id AS TEXT)=:m LIMIT 1"),{"m":mid}).scalar()
                except Exception:m=None
                if isinstance(m,dict):
                    p=_phones(m.get("sender_phone"),m)
                    if p:return p
            # Historical bridge rows may retain sender_phone even when the normalized
            # wa_requirements/wa_messages link is incomplete. Match only exact source
            # identifiers or exact raw text; never infer from unrelated messages.
            if raw:
                try:ev=c.execute(text("SELECT to_jsonb(x) FROM wa_bridge_events x WHERE raw_text=:raw AND NULLIF(BTRIM(sender_phone),'') IS NOT NULL ORDER BY created_at DESC LIMIT 1"),{"raw":raw}).scalar()
                except Exception:ev=None
                if isinstance(ev,dict):
                    p=_phones(ev.get("sender_phone"),ev)
                    if p:return p
    except Exception:pass
    return ""

def _norm_req(obj,source_table=""):
    d=dict(obj or {});ex=_dict(d.get("extracted_fields"));cr=_dict(d.get("clean_record"));merged={};merged.update(ex);merged.update(cr);merged.update({k:v for k,v in d.items() if v not in (None,"",[],{})});src=_classify_source(d.get("source_type") or d.get("source") or source_table);description=_first(merged,["original_message","requirement_message","raw_text","message","requirement_text","requirement","description","additional_points","remarks","notes","content"]) or "";contact=_phones(merged)
    if src=="WHATSAPP" and not contact:contact=_wa_lookup(merged)
    ...
    amin=_first(merged,["area_min_sqft","minimum_area_sqft","minimum_area","min_area_sqft","min_area"]);amax=_first(merged,["area_max_sqft","maximum_area_sqft","maximum_area","max_area_sqft","max_area"]);single=_first(merged,["area_sqft","requirement_sqft","required_area","area"])
    if not amin and single:amin=single
    if not amax and single:amax=single
    return {"date":_first(merged,["created_at","message_timestamp","timestamp","date","captured_at"]),"description":description,"company":_first(merged,["company_name","brand_name","client_company","company","retailer_name","company_brand_person"]),"name":_first(merged,["contact_name","client_name","sender_name","name"]),"contact":contact or "Not captured","location":loc or "Not captured","category":_first(merged,["intended_use","suitable_category","category","property_category","business_category"]),"ptype":_first(merged,["property_type","required_property_type","asset_type"]),"area_min":amin,"area_max":amax,"transaction":_first(merged,["transaction_type","transaction","rent_sale","rent_or_sale","deal_type"]),"budget":_first(merged,["budget_max","budget","sale_budget","rent_budget","budget_raw","budget_max_inr"]),"floor":_first(merged,["floor","preferred_floor","floor_preference","entry_status","additional_preferences"]),"verification":_first(merged,["verification_status","classification","status"]) or "UNVERIFIED","source":src if src!='OTHER' else source_table,"source_id":_first(merged,["source_pk","wa_requirement_id","requirement_id","record_id","id"]),"canonical_id":_first(merged,["canonical_id","master_requirement_id","master_id"]),"gate_id":_first(merged,["gate_id","id"]) if source_table=="pi_requirement_gate_v1191" else ""}
...
def _action(r,source):
    ...
    if src=="MANUAL" and gid:bits.append(f"<a class='btn' href='/alliance/final/requirements/edit-manual?gate_id={_e(gid)}'>Edit</a>")
...
def _requirements_page(engine,source,q=""):
    ...
    return _page(("Master Requirements" if source=='MASTER' else source.title()+" Requirements"),f"<div class=card><form><input name=q value='{_e(q)}' placeholder='Search requirements'> <button>Search</button></form><b>{len(rows)}</b> requirements. Same Master field contract for every source.</div><div class=tablebox><table><thead><tr>{''.join('<th>'+h+'</th>' for h in heads)}</tr></thead><tbody>{''.join(trs) if trs else '<tr><td colspan=17>No requirements found.</td></tr>'}</tbody></table></div>")
def _requirement_hub():
    cards=[("Master Requirements","master"),("WhatsApp","whatsapp"),("Newspaper","newspaper"),("Magazine","magazine"),("Manual","manual")];return _page("Requirement Databases","<div class=grid>"+"".join(f"<div class=card><h3>{t}</h3><a class=btn href='/alliance/final/requirements/{s}'>Open</a>{('<br><br><a class=btn href=/alliance/final/requirements/add-manual>+ Add Requirement</a>' if s=='manual' else '')}</div>" for t,s in cards)+"</div>")
def _manual_page():return _page("Manual","<div class=grid><div class=card><a class=btn href=/property-manual>+ Add Property</a></div><div class=card><a class=btn href=/alliance/final/requirements/add-manual>+ Add Requirement</a></div></div>")
def _whatsapp_requirements():
    try:
        import whatsapp_live_bridge as live;we=live.wa_engine
        ...
        trs=[]
        for raw in data:
            d=_dict(raw);r=_norm_req(d,"WHATSAPP");r["contact"]=_phones(d.get("contact_phone"),d.get("recovered_sender_phone")) or _wa_lookup(d) or "Not captured";r["date"]=d.get("req_time") or r["date"];r["description"]=d.get("raw_text") or r["description"];vals=[r["date"],r["description"],r["company"],r["name"],r["contact"],r["location"],r["category"],r["ptype"],r["area_min"],r["area_max"],r["transaction"],r["budget"],r["floor"],r["verification"],"WHATSAPP",r["source_id"]];cells=[f"<td>{_e(_shown(v))}</td>" for v in vals];cells.append(f"<td>{_action(r,'WHATSAPP')}</td>");trs.append("<tr>"+"".join(cells)+"</tr>")
        heads=["Date / Time","Requirement / Description","Client / Company","Contact Name","Contact No.","Location","Category / Purpose","Property Type","Area Min","Area Max","Rent / Sale","Budget","Floor / Preference","Verification","Source","Source ID","Action"]
        return _page("WhatsApp Live Requirements",f"<div class=card><b>{len(data)}</b> active requirements. Contact recovery checks requirement → linked message → exact bridge event.</div><div class=tablebox><table><thead><tr>{''.join('<th>'+h+'</th>' for h in heads)}</tr></thead><tbody>{''.join(trs)}</tbody></table></div>")
...
def register(core,requirement_app=None,served_app=None):
    ...
    return {"status":"REGISTERED","version":VERSION,"manual_edit_action":"GATE_ROWS","whatsapp_sender_contact":"REQUIREMENT_THEN_MESSAGE_THEN_EXACT_BRIDGE_EVENT","master_contact_recovery":"WHATSAPP_SOURCE_ROWS","database_changed":False}
