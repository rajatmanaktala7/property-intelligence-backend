...
def upsert_hospitality(engine,payload,source):
    ...
    phone=phone[-10:] if phone else None

    canonical=_key(name,phone,payload.get("website"),payload.get("location"))
        ...
        hid=c.execute(text("""
          ...
          ON CONFLICT(canonical_key) DO UPDATE SET
            ...
            category=CASE WHEN EXCLUDED.category='OTHER'
                     THEN ai_hospitality_entity.category ELSE EXCLUDED.category END,
...
def run_discovery(engine,category,location="Delhi NCR",count=8):
    ...
    with engine.begin() as c:
        run_id=c.execute(text("""
          INSERT INTO ai_hospitality_run_history(
            category,query_text,provider,status
          )
...
def adopt_legacy_tables(engine):
    ...
    candidates=[
        ...
        if any(k in t.lower() for k in [
            "hospital","restaurant","banquet","cafe","lounge","club","guest","hotel"
        ])
        ...
        try:
            ...
            for idx,row in enumerate(rows,1):
                ...
                name=next((d.get(k) for k in [
                    "business_name","brand_name","restaurant_name",
                    "hotel_name","name","company_name"
                ] if d.get(k)),None)
                ...
                phone=next((d.get(k) for k in [
                    "contact_phone","phone","contact_no",
                    "contact_number","mobile","whatsapp"
                ] if d.get(k)),None)
                ...
                upsert_hospitality(
                    engine,
                    {
                        ...
                        "location":location,
                        "city":d.get("city"),
                        "contact_name":d.get("contact_name") or d.get("contact_person"),
                        "contact_phone":phone,
                        "email":d.get("email"),
                    ...
                    },
                    {
                        ...
                        "source_name":table,
                        "source_record_id":str(d.get("id") or d.get("prospect_id") or idx),
                        "raw_payload":d,
                    }
                )
            ...
            details.append({"table":table,"rows_adopted_or_merged":table_count})
...
def register_v31_hospitality_routes(core):
    ...
    # ZERO schema DDL during startup.
    ...
    def status(req:Request):
        ...
        if not ready:
            return {
                ...
                "next_step":"POST /api/v3/hospitality/setup",
            }

        with engine.connect() as c:
            entity_count=int(c.execute(
                text("SELECT COUNT(*) FROM ai_hospitality_entity")
            ).scalar() or 0)

            source_count=int(c.execute(
                text("SELECT COUNT(*) FROM ai_hospitality_source_history")
            ).scalar() or 0)

            run_count=int(c.execute(
                text("SELECT COUNT(*) FROM ai_hospitality_run_history")
            ).scalar() or 0)
        ...
        return {
            ...
            "source_history_rows":source_count,
            "run_history_rows":run_count,
            "startup_schema_ddl":False,
            "persistent_storage":True,
    ...
    def entities(req:Request,category:str="",search:str="",limit:int=100):
        ...
        if search:
            clauses.append(
                "(LOWER(business_name) LIKE :q OR LOWER(COALESCE(location,'')) LIKE :q)"
            )
        ...
        return {
            ...
            "schema_ready":True,
            "count":len(rows),
            "entities":[dict(x) for x in rows],
        }

    @app.get("/api/v3/hospitality/sources/{hospitality_id}")
    def sources(hospitality_id:int,req:Request,limit:int=100):
        if hasattr(core,"need_login"):
            core.need_login(req)
        ...
        with engine.connect() as c:
            rows=c.execute(text("""
              SELECT source_history_id,source_type,source_name,source_url,
                     source_record_id,evidence_text,seen_at
              FROM ai_hospitality_source_history
              WHERE hospitality_id=:id
              ORDER BY source_history_id DESC
              LIMIT :lim
            """),{
                "id":int(hospitality_id),
    ...
    def runs(req:Request,limit:int=50):
        ...
        with engine.connect() as c:
            rows=c.execute(text("""
              SELECT run_id,category,query_text,provider,status,
                     fetched_count,inserted_or_updated,error_message,
