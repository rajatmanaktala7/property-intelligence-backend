...
def register(wrapped):
 ...
 for name,mod,arg in [
  ('renderer','alliance_cre_os_v1000',core),
  ('cre11','alliance_cre_os_v1100',core),
  ('auth','alliance_browser_auth_v1140',wrapped),
  ('manual_restore','alliance_manual_database_restore_v1150',wrapped),
  ('property_sources','alliance_unified_property_sources_v1160',wrapped),
  ('full_property_database','alliance_full_property_database_v1170',wrapped),
  ('cre1171','alliance_cre_os_v1171',wrapped),
  ('cre1172','alliance_cre_os_v1172',wrapped),
 ]:
  try:
   result[name]=__import__(mod).register(arg)
  except Exception as ex:
   result[name+'_error']=f'{type(ex).__name__}: {ex}'
