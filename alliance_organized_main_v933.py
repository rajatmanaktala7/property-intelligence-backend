...
VERSION="9.3.3-FAST-STATS-NONBLOCKING"
...
def _fast_total(e,entity,source):
    ...
    if source=="MASTER":
        ...
        WHERE
          ...
          OR UPPER(COALESCE({alias}.canonical_id,'')) LIKE :idpat
        """
        params={"etype":entity,"pat":f"%{token}%","idpat":f"%{token[:3]}%"}
    try:
        with e.connect() as c:
...
def _hub(e,entity):
    cards=[]
    for s in SOURCES:
        total=_fast_total(e,entity,s)
        label=f"{s.title()} Database" if entity=="PROPERTY" else f"{s.title()} Requirements"
        url=f"/alliance/final/database/{s.lower()}" if entity=="PROPERTY" else f"/alliance/final/requirements/{s.lower()}"
        cards.append(f'<div class="dbcard"><b>{_e(label)}</b><div class="n">{total}</div><small>Live canonical count</small><br><a class="btn good" href="{url}">Open</a></div>')
    return "".join(cards)
def _dashboard(e):
