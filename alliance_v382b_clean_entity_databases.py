...
def register(core):
    ...
    @router.post("/api/v382/setup")
    def setup(req:Request):
        need_login(req)
        try:
            _ensure_schema(engine)
            return {"version":VERSION,"status":"READY","startup_schema_ddl":False}
    ...
    def sync_source(source:str,req:Request,limit:int=Query(5000,ge=1,le=10000)):
        ...
        if src=="MANUAL":
            # The authoritative Manual Property Database in the current Alliance app is pi_properties.
            # Keep fallbacks for older deployments, but always prefer pi_properties.
            for t in ["pi_properties","ai_manual_property_final","pi_manual_property_final","manual_property_final"]:
                if _exists(engine,t):
                    table=t
                    break
        ...
        with engine.begin() as c:
            for i,d in enumerate(rows):
                ...
                if src=="NEWSPAPER":
                    ...
                    # Current Alliance manual inventory lives in pi_properties.
                    # Description keeps operational remarks + suitable category + nearby brands.
                    desc_parts=[
                        d.get("remarks"),
                        ("Suitable: "+str(d.get("suitable_category"))) if d.get("suitable_category") else None,
                        ("Nearby brands: "+str(d.get("nearby_brands"))) if d.get("nearby_brands") else None,
                        ("Floor: "+str(d.get("floor"))) if d.get("floor") else None,
                        ("Possession: "+str(d.get("possession"))) if d.get("possession") else None,
                        ("Parking: "+str(d.get("parking"))) if d.get("parking") else None,
                    ]
                    desc=" | ".join(str(x) for x in desc_parts if x)

                    area_value=(
                        d.get("available_area_sqft")
                        or d.get("minimum_area_sqft")
                        or d.get("maximum_area_sqft")
                        or d.get("area_sqft")
                        or d.get("available_area")
                        or d.get("area")
                    )

                    owner_name=d.get("owner_name")
                    owner_phone=d.get("owner_contact") or d.get("owner_phone")
                    broker_name=d.get("broker_name")
                    broker_phone=d.get("broker_contact") or d.get("broker_phone")

                    contact_name=owner_name or broker_name or d.get("owner_broker_name")
                    contact_phone=owner_phone or broker_phone or d.get("contact_number")

                    if owner_name and broker_name:
                        contact_name=f"Owner: {owner_name} | Broker: {broker_name}"
                    if owner_phone and broker_phone:
                        contact_phone=f"{owner_phone} | {broker_phone}"

                    row=dict(
                        source_type=src,
                        source_table=table,
                        source_record_id=str(d.get("property_id") or rid),
                        parent_message_id=None,
                        source_name=d.get("source") or "Manual Property Database",
                        property_name=d.get("property_name") or d.get("property_id") or d.get("location") or "Manual Property",
                        description=desc or d.get("property_name") or "",
                        location=d.get("location") or d.get("locality") or d.get("city"),
                        property_type=d.get("property_type"),
                        transaction_type=d.get("rent_or_sale") or d.get("transaction_type") or d.get("rent_sale"),
                        area_sqft=_num(area_value),
                        # pi_properties does not currently have a dedicated numeric monthly-rent column.
                        ...
                        capture_date=d.get("created_at") or d.get("updated_at"),
                        raw_text=desc or str(d.get("remarks") or "")
                    )
                _upsert(c,row)
                count+=1
    ...
    @router.post("/api/v382/entity/{entity_code}")
    ...
    @router.delete("/api/v382/entity/{entity_code}")
    ...
    @router.get("/v382/property-databases",response_class=HTMLResponse)
    def page(req:Request):
        if not page_role_or_redirect(req):
            return RedirectResponse("/login",303)
        html=Path(__file__).with_name("v382_page.html").read_text(encoding="utf-8")
